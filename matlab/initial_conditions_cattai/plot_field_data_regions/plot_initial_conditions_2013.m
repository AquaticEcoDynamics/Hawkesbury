...
shpfile = '..\..\modeltools\gis\Cattai_Zones_utm.shp';

% Date
sdate = datenum(2013,07,01);

datearray2 = [datenum(2010,01,01) datenum(2019,01,01)];
...
for i = 1:length(vars)
    ...
    for j = 1:length(zones)
        ...
        if ~isempty(init.(vars{i}).(['zone',num2str(j)]).Date)
            ...
            plot(xdata_d,ydata_d,'.K');hold on
            yl = get(gca,'ylim');
            
            bb = find(~isnan(ydata_d));
            if ~isempty(bb)
                mean_val = mean(ydata_d(bb));
            else
                mean_val = 0;
            end
            ...
            xlim([datearray2(1) datearray2(end)]);
            
            set(gca,'xtick',datearray2,'xticklabel',datestr(datearray2,'dd-mm'));
            %datetick('x','mm-yy');
