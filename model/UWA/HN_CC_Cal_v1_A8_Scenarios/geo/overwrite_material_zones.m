...
input_file = 'Hawkesbury_cattai_joined_v1.2dm';
output_file = 'Hawkesbury_cattai_joined_v1_MAT.2dm';

shpfile = 'HN_CC_MAT.shp';


shp = shaperead(shpfile);

for i = 1:length(shp)
    switch shp(i).Material
        case 101
            shp(i).Material = 40;
        case 102
            shp(i).Material = 41;
...
for i = 1:length(X)
    ...
    for jj = 1:length(shp)
        ...
        if inpol == 1
            % And the value hasn't been changed
            original_val = shp(jj).Material;
%             if mat_val == 1
%                 
%                 mat_val = 5;%shp(jj).Material + 999;
%             end
%             
        end
